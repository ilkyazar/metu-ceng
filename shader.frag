...
void main(){
    ...
    float NormalDotLight = clamp(dot(vertexNormal, toLightVec), 0, 1); //for diffuse component    
    vec3 H = normalize(toLightVec + toCameraVec)
    float NormalDotH = clamp(dot(vertexNormal, H), 0, 1); //for specular component

    vec3 diffuseColor = diffuseLightColor * diffuseCoeff * NormalDotLight;
    vec3 specularColor = specularLightColor * specularCoeff * pow(NormalDotH, specularExp);
    vec3 ambientColor = ambientLightColor * ambientCoeff;
    ...
    */
    vec3 totalColor = ambientColor + specularColor + diffuseColor;
    vec3 vertexColor3 = clamp(textureColor3 * totalColor, 0.0, 1.0);
